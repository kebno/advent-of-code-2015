...
int main(int argc, char * argv[]) {
	...
	size_t xposS = 0, yposS = 0, xposR = 0, yposR = 0;
	size_t ix, iy;
	...
	int instructions[2] = {0,0};
	size_t instruction_count = 0;
	while (((instructions[0] = fgetc(fp)) != EOF) && ((instructions[1] = fgetc(fp)) != EOF)) {
		...
		switch (instructions[1]) {
			...
			case ' ':
				break;
			default:
				fprintf(stderr, "PROBLEM. Robot Santa doesn't understand this instruction: \"%c\"\n", instructions[0]);
				fprintf(stderr, "Fix it, fix it, fix it, and Try Again!!!!\n");
				exit(EXIT_FAILURE);
		}
		if ((xposR + yposR * Y) > ((X-1) * (Y-1))) {
			fprintf(stderr, "Robot Santa went out of the expected grid.\n");
			fprintf(stderr, "Please change the sizes \"X\" and \"Y\" in the source code.\n");
			exit(EXIT_FAILURE);
		}
		house_grid[xposR + yposR * Y] += 1;
