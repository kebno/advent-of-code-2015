/* DAY 6, Part 2
 * Change light brightness for Santa, create image of it.
...
int main(int argc, char * argv[]) {
	...
	size_t ix = 0, iy = 0;
	int light_field[N_LIGHTS][N_LIGHTS] = {{0}};
	unsigned char image_data[N_LIGHTS][N_LIGHTS] = {{0}};
	...
	while (fscanf(input_fp, "%c%zd,%zd,%zd,%zd\n", &light_operator,&X1,&Y1,&X2,&Y2) == 5) {
		...
		for (ix = X1; ix <= X2; ix++) {
			for (iy = Y1; iy <= Y2; iy++) {
				// Our operators now have this meaning:
				//  +: increase brightness by 1
				//  -: decrease brightness by 1 (to min of 0)
				...
				switch(light_operator) {
					...
					case '-':
						light_field[ix][iy] -= 1;
						if (light_field[ix][iy] < 0) light_field[ix][iy] = 0;
						break;
					case '|':
						light_field[ix][iy] += 2;
						break;
	...
	for (ix = 0; ix < N_LIGHTS; ix++) {
		for (iy = 0; iy < N_LIGHTS; iy++) {
			total_brightness += light_field[ix][iy];
			if (light_field[ix][iy] > 255) fprintf(stderr, "There was a value greater than 255.\n");
			// Scale the value for output to PGM format
			image_data[ix][iy] =  light_field[ix][iy];
		}
	}

	FILE * image_fp = fopen("lights_for_santa_2.pgm", "wb");
	fprintf(image_fp, "P5\n%d %d\n255\n", N_LIGHTS, N_LIGHTS);
	fwrite(image_data, sizeof(image_data), 1, image_fp);
	fclose(image_fp);

	printf("Total brightness is %zu\n", total_brightness);
	return(0);
}
